...
internal class Program
{
        ...
        while (isRunning)
        {
            ...
            switch (choice)
            {
                ...
                case "19":
                    ...
                    break;
                case "20":
                    try { await GenerateHeavyReport(); }
                    catch (InvalidStudentDataException ex) { WriteError(ex.Message); }
                    catch (StudentNotFoundException ex) { WriteError(ex.Message); }
                    catch (Exception ex) { WriteError(ex.Message); }
                    break;
                case "21":
                    try { AnalyzeStudentClass(); }
                    catch (InvalidStudentDataException ex) { WriteError(ex.Message); }
                    catch (StudentNotFoundException ex) { WriteError(ex.Message); }
                    catch (Exception ex) { WriteError(ex.Message); }
                    break;
                case "22":
                    try { ShowDeveloperMetadata(); }
                    catch (InvalidStudentDataException ex) { WriteError(ex.Message); }
                    catch (StudentNotFoundException ex) { WriteError(ex.Message); }
                    catch (Exception ex) { WriteError(ex.Message); }
                    break;
    ...
    {
        Console.WriteLine("=== Група К-320 | Сидорчук Єлизавета | Практична робота №9 ===");
    ...
    private static async Task SaveOrLoadDataAsync()
    {
        ...
        if (choice == "1")
        {
            await Group.SaveToFileAsync(DataFilePath);
            Console.WriteLine($"Дані збережено у файл: {DataFilePath}");
        }
        else if (choice == "2")
        {
            await Group.LoadFromFileAsync(DataFilePath);
    ...
    private static async Task GenerateHeavyReport()
    {
        Console.WriteLine("Генерація звіту почалась...");
        string report = await Group.GenerateHeavyReportAsync();
    ...
    private static void ShowDeveloperMetadata()
    {
